...
const projectData = [
    {
		title: "小兔快跑游戏",
		tags: ["Agent开发", "unity", "游戏开发"],
		video: "videos/xiaotu_demo.mp4",
		desc: `<h3>项目简介</h3>
		<p>本项目是一款利用Agent开发技术，基于Unity引擎与Python Flask后端搭建的3D萌兔闯关游戏，实现了多账号身份鉴权、客户端独立表现隔离以及无数据库环境下的JSON云端数据持久化闭环。</p>
		
		<h3>项目核心设计报告</h3>
		<p>项目设计详情，点击下方卡片即可全屏在线审阅：</p>
		    
		<div class="pdf-trigger-card" data-pdf="pdfs/xiaotu_report.pdf">
		    <div class="pdf-icon">📄</div>
		    <div class="pdf-info">
		        <h4>小兔快跑游戏设计说明书.pdf</h4>
		...
		<h3>核心技术亮点</h3>
		<p>独立完成3D场景跑酷闯关游戏的玩法设计与系统开发，设计3个不同场景路径的关卡，构建角色移动、血量、伤害、道具、背包、任务、UI及胜负判定等核心Gameplay系统。
		基于Unity C#完成PlayerController、PlayerHealth、InventoryManager、QuestManager、GameStatusManager、UIManager等模块开发，实现玩家移动、怪物/陷阱伤害、道具回血、任务目标及关卡结算等完整玩法闭环。
		采用Unity客户端 + Python Flask服务端架构，实现账号登录、HTTP通信及多账号数据隔离；对玩家背包、关卡、配置等数据进行持久化存储，使不同账号登录后能够加载对应的游戏状态。
		使用Cursor Agent辅助代码生成、架构重构与Debug，解决跨场景数据管理、网络请求异常等开发问题，完成项目从单机玩法Demo到网络联动及数据持久化版本的迭代。
		</p>`
    },
    {
        title: "虚实客家",
