...
// 手机端触屏划动（Swipe）旋转控制
// ==========================================
let touchStartX = 0; // 记录手指刚碰大屏幕时的 X 坐标
let touchEndX = 0;   // 记录手指离开屏幕时的 X 坐标

// 1. 手指按下屏幕的一瞬间
window.addEventListener('touchstart', (e) => {
    // 如果当前点开的是弹窗，则不触发 3D 圆环旋转
    if (modal.style.display === 'flex' || pdfModal.style.display === 'flex') return;
...
window.addEventListener('touchend', (e) => {
    ...
    // 🎯 设定一个滑动阈值（比如 30px），防止用户只是轻点屏幕时误触发旋转
